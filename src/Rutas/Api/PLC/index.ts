    ...
    try {
        ...
        if (resultados) {
            ...
            if (data.length > 1) {
                ...
                if (Partida < 300000 || Partida > 399999) {
                    errorMsg = `MALA PARTIDA ${Partida}`;
                    resultados = false
                };
            } else if (data.length == 0) {
                errorMsg = `SIN DATOS PARA MOSTRAR`;
                resultados = false
            }
        } else {
            errorMsg = "SIN PING AL EQUIPO";
        }

        res.json({ error: errorMsg != "", resultados, errorMsg });
    } catch (error) {
        res.json({
            error: true,
            resultados: false,
