...
const Login = async (cuil:string, clave: string): Promise<object> => {
	try {
		...
		if (!result) throw new CustomError("No se encuentra registrado un cliente con las credenciales indicadas. Si es la primera vez, debe registrarse haciendo click en 'Registrarse'.");;

		if (!result["Habilitado"]) throw new CustomError("No se encuentra habilitado para consultar las Hojas de Seguridad.");
