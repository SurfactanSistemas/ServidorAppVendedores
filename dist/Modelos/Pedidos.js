...
exports.getPendienteDetalle = exports.getPendientes = exports.getDetalles = exports.getAll = void 0;
const sql = __importStar(require("mssql"));
const lodash_1 = __importDefault(require("lodash"));
const getAll = async (vendedor, fecha) => {
    try {
        fecha = fecha.replace(/-/g, "/");
        const WFiltroVendedor = vendedor == 99 ? "" : ` and c.Vendedor = ${vendedor}`;
        const { recordset } = await new sql.Request()
            .query(`select hr.Cliente, c.Razon, c.Vendedor, DesVendedor = CASE WHEN c.Vendedor = 1 then 'Directo' ELSE LTRIM(RTRIM(o.Descripcion)) END , hr.Pedido, hr.Fecha, hr.Hoja, count(p.Terminado) CantItems from HojaRuta hr INNER JOIN Pedido p ON p.Pedido = hr.Pedido and p.HojaRuta = hr.Hoja
        ...
        let res = lodash_1.default(recordset)
            ...
            Datos: lodash_1.default(Clientes)
                ...
                Datos: lodash_1.default(Pedidos)
                    .groupBy("Pedido")
                    .map((Datos, ped) => ({
                    Pedido: ped,
...
const getDetalles = async (hojaRuta, pedido) => {
    try {
        const { recordset } = await new sql.Request()
            .query(`SELECT p.Pedido, p.Renglon, p.Cliente, RTRIM(LTRIM(c.Razon)) Razon, Producto = CASE WHEN p.TipoPro = 'M' THEN p.Articulo ELSE p.Terminado END, 
        ...
        const res = lodash_1.default(recordset)
            .groupBy("Pedido")
            .map((Productos, ped) => ({
            Pedido: ped,
...
const getPendientes = async (vendedor, soloAutorizado) => {
    try {
        const WFiltroSoloAutorizado = soloAutorizado == 1 ? "" : ` And ISNULL(p.Autorizo, '') <> 'N'`;
        const WFiltroVendedor = vendedor == 99 ? "" : ` AND c.Vendedor = '${vendedor}'`;
        const { recordset } = await new sql.Request()
            .query(`select p.Pedido, p.Cliente, RTRIM(LTRIM(c.Razon)) Razon, c.Vendedor, DescVendedor = CASE WHEN c.Vendedor = '1' THEN 'Directo' ELSE LTRIM(RTRIM(o.Descripcion)) END, p.Fecha As FechaPedido,
        ...
        const res = lodash_1.default(recordset)
            ...
            Datos: lodash_1.default(Pedidos)
                ...
                Pedidos: lodash_1.default(Pedidos)
                    .groupBy("Pedido")
                    .map((Pedidos, ped) => ({
                    Pedido: ped,
...
const getPendienteDetalle = async (pedido) => {
    try {
        const { recordset } = await new sql.Request()
            .query(`select p.Pedido, p.Cliente, RTRIM(LTRIM(c.Razon)) Razon, c.Vendedor, DescVendedor = CASE WHEN c.Vendedor = '1' THEN 'Directo' ELSE LTRIM(RTRIM(o.Descripcion)) END, p.Fecha As FechaPedido,
                                             p.FecEntrega As FechaEntrega, Producto = CASE WHEN p.TipoPro = 'M' THEN p.Articulo ELSE p.Terminado END,
...
exports.getPendienteDetalle = getPendienteDetalle;
