...
const Precios_1 = require("./../../Modelos/Precios");
const router = express.Router();
router.get('/:vendedor/:anio', async (req, res) => {
    try {
        let { vendedor } = req.params;
        const resultados = await Precios_1.getAll(vendedor);
...
router.get('/:vendedor/:anio/:cliente', async (req, res) => {
    try {
        ...
        const resultados = await Precios_1.getAllCliente(vendedor, cliente);
        res.json({ error: false, resultados });
    }
    catch (err) {
        res.json({
